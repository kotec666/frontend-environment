...
| Cross-Origin-Resource-Policy  | <br>• `same-site Разрешает загрузку только с того же сайта (но не обязательно того же origin). Пример: sub.example.com → example.com.` <br>• `same-origin: Жёстче: разрешает загрузку только с идентичного origin (протокол + домен + порт должны совпадать).` <br>• `cross-origin: Отключает защиту, разрешая загрузку с любых доменов (не рекомендуется для приватных ресурсов). `  | `Cross-Origin-Resource-Policy: same-site`    | [Страница на MDN](https://developer.mozilla.org/en-US/docs/Web/HTTP/Guides/Cross-Origin_Resource_Policy)                                                                                                                      |
